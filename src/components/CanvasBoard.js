    ...
    >
      ...
      {hasSigned && !showWelcome && !isExporting && (
        <>
          ...
          >
            <CButton label="<" onClick={() => setShowWelcome(true)} />
          </div>
          ...
          >
            <CButton 
              label={isPanning ? 'Draw' : 'Pan'} // Reverted label
              onClick={togglePanning} 
              // Reverted disabled logic
              disabled={isEnteringCaption || isDrawing}
            />
          </div>
        </>
      )}
      ...
      {showWelcome && (
        <div className="welcome-card-wrapper">
          <WelcomeCard sign="Sign the Canvas" onSign={handleSign} />
        </div>
      )}
